...
void Main ()
{
	...
	if (Fork () == 0) {			
		...
		if (Fork () == 0) {			/* Car 10 (child process) */
			Wait (sem);
			Delay (0);
			driveRoad (WEST, 60);
			Signal (sem);
		...
		if (Fork () == 0) {			/* Car 11 (child process) */
			Wait (sem);				/* THIS SHOULD NOT PRINT */
			Delay (0);
			driveRoad (EAST, 60);
			Signal (sem);
			Exit ();
